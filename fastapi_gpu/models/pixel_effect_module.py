...
class PixelEffectModule(nn.Module):
    ...
    def forward(self, rgb:Tensor, param_num_bins=4, param_kernel_size=3, param_pixel_size=32) -> Tensor:
        """
        ...
        intensity = self.create_mask_by_idx(intensity_idx, max_z=param_num_bins).to(self.device)
        intensity = torch.permute(intensity, dims=[2, 0, 1]).unsqueeze(dim=0)

        r, g, b = r * intensity, g * intensity, b * intensity

        kernel_conv = torch.ones([param_num_bins, 1, param_kernel_size, param_kernel_size]).to(self.device)
        r = F.conv2d(input=r, weight=kernel_conv, padding=(param_kernel_size - 1) // 2, stride=param_pixel_size, groups=param_num_bins, bias=None)[0, :, :, :].to(self.device)
        g = F.conv2d(input=g, weight=kernel_conv, padding=(param_kernel_size - 1) // 2, stride=param_pixel_size, groups=param_num_bins, bias=None)[0, :, :, :].to(self.device)
        b = F.conv2d(input=b, weight=kernel_conv, padding=(param_kernel_size - 1) // 2, stride=param_pixel_size, groups=param_num_bins, bias=None)[0, :, :, :].to(self.device)
        intensity = F.conv2d(input=intensity, weight=kernel_conv, padding=(param_kernel_size - 1) // 2, stride=param_pixel_size, groups=param_num_bins, bias=None)[0, :, :, :].to(self.device)
